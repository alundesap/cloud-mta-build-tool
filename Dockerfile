...
RUN apt-get update && \
    apt-get install --yes --no-install-recommends curl git unzip zip vim build-essential python2.7 && \
    ...
    echo "[INFO] untarring Golang." && \
    tar -xzf go${GO_VERSION}.linux-amd64.tar.gz && \
    mv go /usr/local && \
    mkdir -p "$GOPATH/src" "$GOPATH/bin" && chmod -R 777 "$GOPATH" && \
    mkdir -p ${GOPATH}/src ${GOPATH}/bin && \

    echo "[INFO] update maven home." && \
    # update maven home
    ...
    echo "[INFO] handle users permission." && \
    ...
    echo "[INFO] install tools and python." && \
